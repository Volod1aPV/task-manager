...
const schema = z.object({
  title: z.string().min(1, 'Název je povinný').max(100),
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      <nav style={{background:'var(--bg-card)',borderBottom:'1px solid var(--border)',padding:'0 32px',height:'60px',display:'flex',alignItems:'center',gap:'16px',position:'sticky',top:0,zIndex:10}}>
        <Link href={`/tasks/${id}`} style={{color:'var(--text-muted)',fontSize:'13px'}}>← Zpět</Link>
        <div style={{width:'1px',height:'20px',background:'var(--border)'}} />
        <span style={{fontSize:'14px',fontWeight:'600',color:'var(--text-primary)'}}>Upravit úkol</span>
      </nav>

      <div style={{maxWidth:'560px',margin:'0 auto',padding:'40px 16px'}} className="fade-in">
        <div style={{marginBottom:'28px'}}>
          <h1 style={{fontSize:'22px',fontWeight:'800',color:'var(--text-primary)',marginBottom:'4px'}}>Upravit úkol</h1>
          <p style={{color:'var(--text-faint)',fontSize:'14px'}}>Změňte informace o úkolu</p>
        </div>
        ...
        <div className="card" style={{padding:'28px'}}>
          <form onSubmit={handleSubmit(onSubmit)} style={{display:'flex',flexDirection:'column',gap:'20px'}}>
            ...
            <div>
              ...
              <textarea {...register('description')} rows={3} className="input-field" style={{resize:'none'}} />
            </div>
            ...
            <div style={{display:'flex',gap:'10px',paddingTop:'4px'}}>
              <Link href={`/tasks/${id}`} className="btn-secondary" style={{flex:1,padding:'11px',fontSize:'14px',borderRadius:'10px'}}>Zrušit</Link>
